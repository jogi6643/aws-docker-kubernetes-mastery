  ...
  "comparisons": [
    {
      ...
      "headers": ["Aspect", "Docker Standalone", "Kubernetes (K8s)"],
      "rows": [
        ["Primary Role", "Builds, packages, and runs containers on a single host", "Orchestrates and manages container clusters across multi-node fleets"],
        ["Scope", "Single Node / Local Machine", "Distributed Multi-Host Cluster (hundreds of nodes)"],
        ["Auto-Healing", "Restarts dead containers on same host only", "Reschedules pods to completely different healthy nodes across cluster"],
        ["Scaling Mechanism", "Manual scaling via CLI (`docker scale` in Swarm)", "Automatic Horizontal Pod Autoscaler (HPA) based on CPU/RAM metrics"],
        ["Service Discovery & LB", "Basic port forwarding on host interface", "Built-in CoreDNS and Service virtual IPs (ClusterIP, NodePort, LoadBalancer)"],
        ["Zero-Downtime Rollouts", "Requires manual script or blue/green switching", "Declarative RollingUpdate strategies with instant one-command rollbacks"]
      ]
    },
    {
      "id": "rc-vs-rs-vs-dep",
      ...
      "headers": ["Controller", "ReplicationController", "ReplicaSet", "Deployment"],
      "rows": [
        ["Generation", "1st Generation (Legacy)", "2nd Generation", "3rd Generation (Current Standard)"],
        ["Selector Support", "Equality-based only (`env = prod`)", "Set-based (`env in (prod, stage)`)", "Set-based (`matchExpressions`)"],
        ["Rolling Updates", "Requires client-side kubectl rolling-update", "No native rolling update support", "Native server-side zero-downtime RollingUpdate"],
        ["Rollback Capability", "No", "No", "Yes (`kubectl rollout undo` with full revision history)"],
        ["Best Practice", "Deprecated / Do not use", "Managed implicitly by Deployments", "Always recommended for stateless production applications"]
      ]
    },
    {
      "id": "service-types",
      ...
      "headers": ["Type", "ClusterIP", "NodePort", "LoadBalancer", "Ingress"],
      "rows": [
        ["Accessibility", "Internal Cluster Only", "External via Node IP & Port", "External via Dedicated Cloud IP", "External Layer 7 HTTP/HTTPS Router"],
        ["Port Range", "Standard ports (80, 443, 3306)", "High range only (30000 - 32767)", "Standard ports (80, 443)", "Standard ports (80, 443) with SSL termination"],
        ["Cloud Dependency", "None", "None", "Requires AWS / GCP / Azure", "Requires Ingress Controller (Nginx/Traefik)"],
        ["Cost Factor", "Free (Zero cost)", "Free (Zero cost)", "Costly (One cloud LB provisioned per service)", "Cost-efficient (Single LB routes to 100+ services)"],
        ["Primary Use Case", "Backend databases, Redis, intra-service APIs", "Dev/testing, non-cloud bare metal setups", "Single public services on cloud", "Production microservices, path-based / host-based routing"]
      ]
    },
    {
      "id": "probe-types",
      ...
      "headers": ["Probe", "Liveness Probe", "Readiness Probe", "Startup Probe"],
      "rows": [
        ["Primary Question", "'Is the container alive or deadlocked?'", "'Is the container ready to receive user traffic?'", "'Has the slow-starting application initialized yet?'"],
        ["Failure Action", "Kubelet KILLS and RESTARTS the container", "Kubelet REMOVES pod IP from Service Endpoints (No restart)", "Kubelet KILLS container if it exceeds max startup duration"],
        ["Traffic Impact", "Container restarts, brief interruption", "Existing container stays alive; traffic safely paused", "Disables liveness checks until application starts cleanly"],
        ["Execution Period", "Runs continuously throughout container lifecycle", "Runs continuously throughout container lifecycle", "Runs only during container initial boot phase"]
      ]
    }
  ],
  "quizQuestions": [

    ...
    "comparisons": [
        {
            ...
            "headers": ["Feature", "Security Group (SG)", "Network ACL (NACL)"],
            "rows": [
                ["Level", "Instance / ENI Level", "Subnet Level"],
                ["State Tracking", "Stateful (Return traffic automatically allowed)", "Stateless (Inbound & Outbound evaluated separately)"],
                ["Rule Types", "Supports ALLOW rules only", "Supports both ALLOW and DENY rules"],
                ["Rule Evaluation", "All rules evaluated before deciding", "Numbered rules evaluated sequentially (lowest first)"],
                ["Default State (Custom)", "Denies all inbound, allows all outbound", "Denies all inbound, denies all outbound"],
                ["Blocking Specific IP", "Cannot block specific IP", "Can block specific IP using low rule number"],
                ["Ephemeral Ports", "Automatically handled statefully", "Must explicitly allow outbound TCP 1024-65535"]
            ]
        },
        {
            "id": "alb-vs-nlb",
            ...
            "headers": ["Feature", "Application Load Balancer (ALB)", "Network Load Balancer (NLB)"],
            "rows": [
                ["OSI Layer", "Layer 7 (Application: HTTP, HTTPS, gRPC)", "Layer 4 (Transport: TCP, UDP, TLS)"],
                ["Throughput & Latency", "High throughput, ~10-50ms latency", "Extreme throughput (millions RPS), microsecond latency"],
                ["Static IP per AZ", "No (Uses dynamic DNS hostname)", "YES (Static Elastic IP per Availability Zone)"],
                ["Routing Capabilities", "Path (`/api`), Host, HTTP headers, Query strings", "IP address, Port, and Protocol only"],
                ["WebSockets & HTTP/2", "Yes, natively supported", "Yes (TCP pass-through)"],
                ["Cross-Zone Load Balancing", "Enabled by default (free)", "Disabled by default (can be enabled)"],
                ["AWS WAF Integration", "Yes, direct WAF integration", "No (requires ALB or CloudFront behind it)"],
                ["Primary Use Case", "Websites, microservices, container apps", "Gaming, financial trading, ultra-low latency, static IP whitelisting"]
            ]
        },
        {
            "id": "ebs-vs-efs-vs-s3",
            ...
            "headers": ["Feature", "Amazon EBS", "Amazon EFS", "Amazon S3"],
            "rows": [
                ["Storage Type", "Block Storage", "File Storage (POSIX)", "Object Storage"],
                ["Access Protocol", "Direct NVMe / SCSI block bus", "NFSv4 (TCP Port 2049)", "HTTPS REST API (GET, PUT, DELETE)"],
                ["Instance Attachments", "Single EC2 in single AZ (Multi-Attach on io1/io2)", "1,000s of EC2 Linux instances across multiple AZs", "Infinite concurrent access globally"],
                ["Durability & Scope", "AZ-specific (99.8% to 99.999%)", "Multi-AZ regional (99.999999999%)", "Multi-AZ regional (99.999999999% - 11 9s)"],
                ["OS Boot Support", "Yes (Primary OS boot drive)", "No (Data files only)", "No"],
                ["Capacity Scaling", "Fixed provisioned size (modify on fly)", "Elastic (auto-scales up and down)", "Infinite scale (zero capacity planning)"],
                ["Cost Profile", "$0.08 / GB / mo (gp3)", "$0.30 / GB / mo ($0.025 with IA)", "$0.023 / GB / mo ($0.00099 Glacier Deep)"]
            ]
        },
        {
            ...
            "headers": ["Feature", "Amazon RDS", "Amazon Aurora", "Amazon DynamoDB"],
            "rows": [
                ["Database Model", "Relational (OLTP)", "Cloud-Native Relational (MySQL/PostgreSQL)", "NoSQL (Key-Value & Document)"],
                ["Schema", "Strict SQL Schema", "Strict SQL Schema", "Schemaless (Flexible JSON attributes)"],
                ["Storage Architecture", "EBS volume attached to VM instance", "Shared distributed storage fleet across 3 AZs", "Fully managed distributed SSD partitions"],
                ["Replication & HA", "Multi-AZ (1 synchronous standby in 2nd AZ)", "6 copies across 3 AZs (Quorum write: 4, read: 3)", "Synchronous replication across 3 AZs"],
                ["Read Replicas", "Up to 5 replicas (Async replication lag)", "Up to 15 replicas (Sub-10ms lag, auto-failover)", "Global Tables (Active-Active multi-region)"],
                ["Scaling Dimensions", "Vertical compute resize + Storage autoscaling", "Vertical compute + Aurora Serverless v2", "Horizontal infinite scaling (RCU/WCU / On-Demand)"],
                ["Latency", "Low millisecond", "Low millisecond", "Single-digit millisecond / Microsecond (DAX)"]
            ]
        },
        {
            "id": "sqs-vs-sns",
            ...
            "headers": ["Feature", "Amazon SQS", "Amazon SNS"],
            "rows": [
                ["Communication Model", "Queue (Point-to-Point / Producer-Consumer)", "Pub/Sub (Publish/Subscribe - One-to-Many)"],
                ["Delivery Mechanism", "PULL: Consumers actively poll the queue", "PUSH: SNS immediately pushes events to subscribers"],
                ["Persistence", "Messages retained up to 14 days (default 4 days)", "No persistence; pushed immediately (or discarded)"],
                ["Subscribers", "Single consumer pool pulls individual messages", "Multiple subscribers (Email, SMS, SQS, Lambda, HTTP)"],
                ["Message Ordering", "Standard (best-effort) / FIFO (strict order)", "Standard / FIFO Topics"],
                ["Max Payload", "256 KB (up to 2GB with S3 Extended Client)", "256 KB"],
                ["Synergy Pattern", "Worker queues buffer burst traffic", "Fan-Out: 1 SNS Topic pushes to multiple SQS queues"]
            ]
        },
        {
            ...
            "headers": ["Feature", "AWS NAT Gateway", "AWS NAT Instance (EC2)"],
            "rows": [
                ["Management", "AWS Fully Managed", "Customer Self-Managed"],
                ["High Availability", "Built-in redundancy within AZ (Deploy 1 per AZ)", "None (Single point of failure; requires scripts)"],
                ["Bandwidth", "Scales automatically up to 45 Gbps", "Limited by EC2 instance type bandwidth"],
                ["Maintenance", "Zero OS maintenance or security patches", "Customer must patch OS, iptables, and security"],
                ["Source/Dest Check", "Not applicable (native)", "MUST BE DISABLED in EC2 settings to route traffic"],
                ["Elastic IP Required", "Yes (Allocated on creation)", "Yes (Associated to EC2 ENI)"],
                ["Cost Model", "Hourly charge + per-GB data processing charge", "EC2 instance hourly rate + data transfer out charge"]
            ]
        }
    ],
    "examTips": [
...
window.AWS_CHEATSHEETS = {json.dumps(CHEATSHEETS, indent=2, ensure_ascii=False)};
"""

with open('js/cheatsheets.js', 'w', encoding='utf-8') as f:
    f.write(js_content)

print("Successfully written js/cheatsheets.js!")
